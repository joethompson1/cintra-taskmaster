...
export class JiraRelationshipResolver {
	...
	private maxRelatedIssues: number;
	private defaultMaxDepth: number;

	/**
	 * Initialize the relationship resolver
	 * @param jiraClient - Configured Jira client instance
	 */
	 ...
	 * @param issueKey - The Jira issue key to start from
	 * @param options - Configuration options
	 * @returns Relationship graph with metadata
	 */
	async resolveRelationships(
	...
	): Promise<JiraResponse<RelationshipGraphData>> {
		const {
		...
		try {
			...
			// Build result object
			const result: JiraResponse<RelationshipGraphData> = {
				success: true,
				data: {
			...
			log.error(`Error resolving relationships for ${issueKey}: ${(error as Error).message}`);
			return {
				success: false,
				error: {
					code: 'RELATIONSHIP_RESOLUTION_ERROR',
					message: `Failed to resolve relationships: ${(error as Error).message}`
				}
			};
		}
	}
	 ...
	 * @returns True if relationship already exists
	 */
	private isRelationshipExists(relationships: RelationshipData[], issueKey: string, relationship: string): boolean {
	 ...
	 * @param issue - Current issue being processed
	 * @param relationships - Array to collect relationships
	 * @param visited - Set of visited issue keys to prevent cycles
	 * @param maxDepth - Maximum depth to traverse
	 * @param includeTypes - Relationship types to include
	 ...
	 * @param relationships - Array to collect relationships
	 * @param visited - Set of visited issue keys
	 * @param maxDepth - Maximum depth to traverse
	 * @param includeTypes - Relationship types to include
	 * @param currentDepth - Current traversal depth
	 * @param log - Logger object
	 */
	private async traverseIssueLinks(
		issue: JiraTicket, 
		relationships: RelationshipData[], 
		visited: Set<string>, 
		maxDepth: number, 
		...
		for (const link of rawIssue.data.fields.issuelinks) {
			let relatedIssueKey: string | null = null;
			...
			if (!relatedIssueKey || !relationship || !direction || !includeTypes.includes(relationship)) {
				continue;
			}
			...
			if (!this.isRelationshipExists(relationships, relatedIssueKey, relationship)) {
				relationships.push({
					...
					relationship,
					direction: direction as 'inward' | 'outward' | 'upward' | 'downward',
	 ...
	 * @param log - Logger object
	 */
	private async traverseParentChild(
		issue: JiraTicket, 
		relationships: RelationshipData[], 
		visited: Set<string>, 
		maxDepth: number, 
		...
		if (includeTypes.includes('parent') && issue.parentKey) {
			let parentResult: JiraResponse<JiraTicket> | null = null;
	 ...
	 * @param log - Logger object
	 */
	private async traverseEpicRelationships(
		issue: JiraTicket, 
		relationships: RelationshipData[], 
		visited: Set<string>, 
		maxDepth: number, 
		...
		if (includeTypes.includes('story') && issue.issueType === 'Epic') {
			...
			for (let i = 0; i < epicSearchQueries.length; i++) {
				...
				try {
					...
					log.info(`[EPIC DEBUG] Query result - success: ${storiesResult.success}, data length: ${storiesResult.success ? storiesResult.data.length : 'null/undefined'}`);
					...
					if (storiesResult.success && storiesResult.data && storiesResult.data.length > 0) {
						...
						log.info(`[EPIC DEBUG] Story keys: ${storiesResult.data.map(s => s.jiraKey || (s as any).key).join(', ')}`);
					...
					log.warn(`[EPIC DEBUG] Query exception: ${(error as Error).message}`);
			...
			if (foundStories.length > 0) {
				...
				for (let i = 0; i < foundStories.length; i++) {
					...
					const storyKey = storyIssue.jiraKey || (storyIssue as any).key;
		...
		if (includeTypes.includes('epic')) {
			...
			try {
				...
				log.warn(`[EPIC DEBUG] Error finding epic relationship: ${(error as Error).message}`);
			}
		}
	}
	 ...
	 * @param linkType - Jira link type
	 * @returns Standardized relationship type
	 */
	private mapLinkTypeToRelationship(linkType: string): string {
	 ...
	 */
	private getCachedResult(cacheKey: string): JiraResponse<RelationshipGraphData> | null {
	 ...
	 */
	private setCachedResult(cacheKey: string, result: JiraResponse<RelationshipGraphData>): void {
